...
export default function AlbumParticipationPanel({ albumId }: { albumId: string }) {
  ...
  return <section className="family-panel__section">
    ...
    <ul className="member-list">{data.participants.map((person: any) => <li className="member-card" key={person.id}><div><strong>{person.name}</strong><span className="member-card__role">{person.role === "host" ? "주최자" : "참여자"} · 사진 {person.photo_count}장 · 추억 {person.memory_count}개</span></div></li>)}</ul>
