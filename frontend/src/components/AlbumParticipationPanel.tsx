...
export default function AlbumParticipationPanel({ albumId }: { albumId: string }) {
  ...
  const openPending = async () => {
    ...
    } catch { setNotice("새로 더해진 사진과 한마디를 불러오지 못했어요."); }
  };
  const apply = async () => {
    if (!pending) return;
    setBusy(true); setNotice("새로 더해진 사진과 한마디를 앨범에 담고 있어요.");
    try {
      ...
      setPending(null); setSelected(new Set()); setNotice("새로 더해진 사진과 한마디를 앨범에 담았어요."); refresh();
