-- Rollback for 20260803150000_analytics_contribution_claimed.sql. Emergency use only.
BEGIN;

DELETE FROM public.analytics_events WHERE event_name = 'contribution_claimed';
...
ALTER TABLE public.analytics_events ADD CONSTRAINT analytics_events_event_name_check CHECK (
  event_name IN (
    ...
    'album_limit_reached', 'photo_limit_reached', 'video_dropped'
  )
);
