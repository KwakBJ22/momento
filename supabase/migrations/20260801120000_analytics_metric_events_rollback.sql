-- Rollback for 20260801120000_analytics_metric_events.sql. Emergency use only.
-- Removes rows using the two new names first so the narrower CHECK can apply.
BEGIN;

DELETE FROM public.analytics_events WHERE event_name IN ('invitation_opened', 'album_revisited');
...
ALTER TABLE public.analytics_events ADD CONSTRAINT analytics_events_event_name_check CHECK (
  event_name IN (
    ...
    'upload_failed', 'pdf_failed', 'share_failed'
  )
);
